...
There are currently three supported kinds of repositories:

+ `package-server-with-standard-elm-package-server-api`
+ `package-server-with-personal-zokka-repo-v1.0-package-server-api`
+ `local-directory-of-files-readonly-mirror-of-package-server-v1.0`

#### package-server-with-standard-elm-package-server-api

`package-server-with-standard-elm-package-server-api` refers to package servers
which adhere to the API provided by the usual Elm package server. 

This type of package server requires you to fill in a `repository-url` key and a
`repository-local-name` key. 

+ `repository-url`: The URL of the package server (e.g. `https://package.elm-lang.org`). 
+ `repository-local-name`: The name that is used in the `elm publish` command to
  publish a package to the repository (note: we disallow publishing to the main
  Elm package repo for reasons we outline elsewhere in this document).

```
{
    "repository-type": "package-server-with-standard-elm-v0.19-package-server-api",
    "repository-url": "https://example.com",
    "repository-local-name": "my-local-name"
}
```

#### package-server-with-personal-zokka-repo-v1.0-package-server-api

`package-server-with-personal-zokka-repo-v1.0-package-server-api` refers to
package servers which adhere to a Zokka custom package server (which can be
found here:
[https://github.com/Zokka-Dev/zokka-custom-package-server](https://github.com/Zokka-Dev/zokka-custom-package-server)).

These kinds of repositories allow for an authentication token that controls
whether a client is allowed to read from the repo or to both read and publish to
the repo. 

This type of package server requires you to fill in a `repository-url` key, a
`repository-local-name` key, and a `repository-auth-token` key. 

+ `repository-url`: The URL of the package server (e.g. `https://package.elm-lang.org`). 
+ `repository-local-name`: The name that is used in the `elm publish` command to
  publish a package to the repository (note: we disallow publishing to the main
  Elm package repo for reasons we outline elsewhere in this document).
+ `repository-auth-token`: An authentication token that is used by the server to
  determine whether to only allow reading from the repo or to allow both reading
  and publishing

#### local-directory-of-files-readonly-mirror-of-package-server-v1.0

`local-directory-of-files-readonly-mirror-of-package-server-v1.0` refers to
a local directory which contains a set of packages that we can use directly
instead of needing to spin up a server. This is intended to be the output of a
such as
[https://github.com/Zokka-Dev/elm-mirror-server](https://github.com/Zokka-Dev/elm-mirror-server)
and therefore requires the directory to be in the same format as what is
generated by that tool.

Since this is meant to be a read-only mirror of another package server, we
disallow publishing to this repository and therefore it has no
`repository-local-name`. Instead we only have an additional
`repository-file-path` that needs to be specified.

+ `repository-file-path`: The absolute file path to the location of the
  directory of Elm packages.
